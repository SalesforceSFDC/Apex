## Table of Contents
  ...
  * [Triggers](#triggers)
      * [Common Bulk Trigger Idioms](#common-bulk-trigger-idioms)
...
* Sets can be used to isolate distinct records
* Maps can be used to hold query results organized by

### Bult Trigger for Sample Quoting Application
* Trigger first adds each pricebook entry associated with the OpportunityLineItem records in Trigger.new to a set, ensuring that it contains only distinct elements
* Then it queries the PriceBookEntries for their associated product color, and places the results in a map.
